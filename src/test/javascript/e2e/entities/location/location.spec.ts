...
describe('Location e2e test', () => {
  ...
  describe('Create flow', () => {
    it('should load create Location page', async () => {
      ...
      expect(await updatePage.title.getAttribute('id')).to.match(/clientApp.location.home.createOrEditLabel/);
    });

    it('should create and save Locations', async () => {
      await updatePage.headingInput.sendKeys('5');
      expect(await updatePage.headingInput.getAttribute('value')).to.eq('5');
      ...
      await updatePage.speedInput.sendKeys('5');
      expect(await updatePage.speedInput.getAttribute('value')).to.eq('5');

      // await  selectLastOption(updatePage.tripSelect);
    ...
    describe('Details, Update, Delete flow', () => {
      after(async () => {
        ...
        expect(await deleteDialog.title.getAttribute('id')).to.match(/clientApp.location.delete.question/);
      ...
      it('should load edit Location page, fetch data and update', async () => {
        ...
        await clear(updatePage.headingInput);
        await updatePage.headingInput.sendKeys('6');
        expect(await updatePage.headingInput.getAttribute('value')).to.eq('6');
        ...
        await clear(updatePage.speedInput);
        await updatePage.speedInput.sendKeys('6');
        expect(await updatePage.speedInput.getAttribute('value')).to.eq('6');
