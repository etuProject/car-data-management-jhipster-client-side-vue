...
import TripDetailsPage from './trip-details.page-object';
...
describe('Trip e2e test', () => {
  ...
  let updatePage: TripUpdatePage;
  let detailsPage: TripDetailsPage;
  let listPage: TripComponentsPage;
  let deleteDialog: TripDeleteDialog;
  ...
  it('should load Trips', async () => {
    ...
    listPage = new TripComponentsPage();
  ...
  describe('Create flow', () => {
    it('should load create Trip page', async () => {
      ...
      updatePage = new TripUpdatePage();

      await waitUntilAllDisplayed([updatePage.title, updatePage.footer, updatePage.saveButton]);

      expect(await updatePage.title.getAttribute('id')).to.match(/clientApp.trip.home.createOrEditLabel/);
    });
    ...
    describe('Details, Update, Delete flow', () => {
      after(async () => {
        ...
        deleteDialog = new TripDeleteDialog();
        await waitUntilDisplayed(deleteDialog.dialog);

        expect(await deleteDialog.title.getAttribute('id')).to.match(/clientApp.trip.delete.question/);
      ...
      it('should load details Trip page and fetch data', async () => {
        const detailsButton = listPage.getDetailsButton(listPage.records.first());
        await click(detailsButton);

        detailsPage = new TripDetailsPage();
      ...
      it('should load edit Trip page, fetch data and update', async () => {
